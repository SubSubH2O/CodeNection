...
assert.equal(plans.candidates.length, 2, 'Two different feasible choices');
assert.deepEqual(planWork(original, [task]), plans, 'Deterministic results');
assert.equal(original.tasks.length, 0, 'Preview does not mutate state');
...
assert(plans.candidates[1].movedId);
...
assert.equal(planWork(occupied, [task]).candidates.length, 1, 'Occupied destination cannot be suggested');
...
assert.deepEqual(reducer(completed, { type: 'reset' }).tasks, []);
